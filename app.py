import streamlit as st, pandas as pd, io, locale
...
def get_thai_date(date_obj):
    ...
    return f"{days[date_obj.weekday()]}ที่ {date_obj.day} {months[date_obj.month]} {date_obj.year + 543}"

def get_header_color(t_idx, d_type):
    if d_type == 'Normal':
...
def generate_schedule(DAY_OF_WEEK, LEAVES, CUSTOM_TASKS, PART_TIME, FIX_BREAKS, FIXED_MAIN_TASKS, SICK_PEOPLE, IS_MWF):
    ...
    ft_pharmacists = ['เต้น', 'แอน', 'แม็ค', 'โบ้ท', 'ไม้เอก', 'กิ๊ฟ', 'ฟอร์จูน', 'มิ้ลค์', 'ริน', 'อ๊อฟฟี่', 'ออย', 'บี', 'มายด์', 'ขิม', 'บีม', 'มิ้น', 'ใบเตย', 'จีน่า', 'ปอนด์']
    ...
    active_ft, leave_slots, half_day_leaves = [], set(), set()
    ...
    custom_dict_index, custom_task_slots_count = {}, {p: 0 for p in ft_pharmacists} 
    ...
    for t in range(16):
        ...
        if IS_MWF and (t in break_slots):
            if 'Ver_1' in req_core: req_core.remove('Ver_1')
    ...
    for t in range(16):
        weights = {'จ่ายยา_4':300000, 'จ่ายยา_11':290000, 'Ver_4':50000, 'PS_3':48000, 'Match_C2':47000, 'Ver_5':46000, 'PS_4':44000, 'Ver_6':42000, 'PS_5':40000, 'Ver_7':38000, 'PS_6':36000, 'Ver_8':34000, 'PS_7':32000, 'Ver_9':30000, 'PS_8':28000, 'Ver_10':26000, 'PS_9':24000, 'PS_10':22000}
        if IS_MWF and (t in break_slots):
...
def build_html_table(df, selected_date, DAY_OF_WEEK):
    ...
    num_cols = len(cols)
    html = f"<div id='capture-area' style='background-color: white; padding: 20px; display: inline-block; font-family: \"Sarabun\", \"TH Sarabun New\", sans-serif;'><table style='border-collapse: collapse; width: 100%;'><tr><td colspan='{num_cols}' style='text-align: center; font-size: 26px; font-weight: bold; border: none; padding-bottom: 5px;'>ตารางปฏิบัติงานเภสัชกร ห้องยาชั้น 1 อาคารสมเด็จพระเทพรัตน์</td></tr><tr><td colspan='{num_cols}' style='text-align: center; font-size: 20px; font-weight: bold; border: none; padding-bottom: 15px;'>ประจำ{thai_date_str}</td></tr><tr>"
...
st.markdown("<p style='font-size: 14px; color: gray;'>version 115.2 (UI Update) พัฒนาโดย Niratsai Sukprasert และ Gemini</p>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with st.expander("คลิกเพื่อล็อกเวลาพัก (สูงสุด 5 คน)", expanded=False):
        for i in range(5):
            ...
            with c2: t_b = st.selectbox(f"รอบพัก {i+1}", ["รอบที่ 1", "รอบที่ 2", "รอบที่ 3"], key=f"b_time_{i}")
            if p_b != "ไม่มี":
