...
st.markdown("<p style='font-size: 14px; color: gray;'>version 116.5 (Strict 1-hour limits applied) พัฒนาโดย Niratsai Sukprasert และ Gemini</p>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with st.expander("คลิกเพื่อล็อกเวลาพัก (สูงสุด 5 คน)", expanded=False):
        ...
        for i in range(5):
            ...
            with c2: t_b = st.selectbox(f"รอบพัก", break_choices, key=f"b_time_{i}", label_visibility="collapsed")
            st.divider()
            if p_b != "ไม่มี":
