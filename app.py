...
def generate_schedule(DAY_OF_WEEK, LEAVES, CUSTOM_TASKS, PART_TIME, FIX_BREAKS, FIXED_MAIN_TASKS, SICK_PEOPLE, IS_MWF):
    ...
    for p in all_pharmacists:
        ...
        if p in active_ft:
            ...
            if p in FIX_BREAKS and p in ft_pharmacists:
                req_b = FIX_BREAKS[p]
                for i in range(3):
                    if i == req_b: model.Add(choices[i] == 1)
                    else: model.Add(choices[i] == 0)
            else:
            ...
            for i in range(3):
                ...
                for t in range(*b_groups[i]): 
                    model.Add(x[p, t, 'พัก'] == 1).OnlyEnforceIf(choices[i])
    ...
    for t in range(16):
        ...
        if t < 2: 
            req_core = ['จ่ายยา_6', 'จ่ายยา_7', 'จ่ายยา_8', 'จ่ายยา_9', 'Ver_1', 'Ver_2', 'Ver_3', 'PS_1', 'Match_C']
        elif t == 2: 
    ...
    for t in range(16):
        for i in range(2, 10):
            model.Add(sum(x[p, t, f'PS_{i+1}'] for p in all_pharmacists) <= sum(x[p, t, f'PS_{i}'] for p in all_pharmacists))
        for i in range(4, 10):
    ...
    # 7. กฎ 1 ชั่วโมงเฉพาะงานหลัก (บังคับใช้กับ "ทุกคน" เพื่อไม่ให้ PT ยืนจ่ายยาเกิน 1 ชม.)
    for p in all_pharmacists:
        for t in range(14): 
    ...
    # กฎ 1 ชั่วโมงสำหรับ Matching ของ PT (ห้ามทำ Matching ติดกัน 3 ช่อง)
    for p in pt_pharmacists:
        for t in range(14):
            model.Add(sum(x[p, t+k, 'Matching'] for k in range(3)) <= 2)

    # 7.1 ความยุติธรรมและการจ่ายยาของ FT
    is_disp_7_vars = []
    for p in ft_pharmacists:
        ...
        for d in ['จ่ายยา_6', 'จ่ายยา_7', 'จ่ายยา_8', 'จ่ายยา_9']:
            model.Add(sum(x[p, t, d] for t in range(16)) <= 2)
        ...
        # 🌟 กฎเหล็ก: ลงช่อง 7 ห้ามลง 8 เด็ดขาด (เฉพาะ FT) 🌟
    ...
    # 7.5 เว้นระยะการจ่ายยาอย่างน้อย 1 ชั่วโมง (Soft Constraint โทษหนัก)
    for p in all_pharmacists:
        for t in range(14):
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        for t_idx in range(16):
            ...
            for p in all_pharmacists:
                ...
                if 'Ver PS' in val_str:
                    w_count += 1
                elif 'Ver' in val_str: 
                    x_count += 1
                elif 'จ่าย ' in val_str:
                    try:
                        num = int(val_str.replace('จ่าย ', '').strip())
                        disp_nums.append(num)
                    except:
                        pass
        ...
        return None, "Timeout", "ระบบคิดไม่ทัน กรุณาลดการล็อกงานหลัก หรือจำนวนคนลาลงครับ"
    else:
...
def get_color_style(val):
    ...
    if '/' in val_str and '-' in val_str and val_str[0].isdigit():
        ...
        return base_style + 'background-color: #D5E8D4; color: black;' 
    elif val_str == 'Matching':
        return base_style + 'background-color: #DAE8FC; color: black;' 
    elif 'Match' in val_str: 
        return base_style + 'background-color: #DAE8FC; color: #FF0000; font-weight: bold;' 
    elif 'Ver PS' in val_str:
        return base_style + 'background-color: #E1D5E7; color: black;' 
    elif 'Ver' in val_str: 
...
def build_html_table(df, selected_date, DAY_OF_WEEK):
    ...
    <div id="capture-area" style="background-color: white; padding: 20px; display: inline-block; font-family: 'Sarabun', 'TH Sarabun New', sans-serif;">
        <table style="border-collapse: collapse; width: 100%;">
            <tr>
                <td colspan="{num_cols}" style="text-align: center; font-size: 28px; font-weight: bold; border: none; padding-bottom: 5px;">ตารางปฏิบัติงานเภสัชกร ห้องยาชั้น 1 อาคารสมเด็จพระเทพรัตน์</td>
            </tr>
            <tr>
                <td colspan="{num_cols}" style="text-align: center; font-size: 22px; font-weight: bold; border: none; padding-bottom: 15px;">ประจำ{thai_date_str}</td>
            </tr>
            <tr>
    """
...
st.markdown("""
    ...
    .stMarkdown h1 { margin-top: -1rem !important; padding-bottom: 0rem !important; margin-bottom: 0.5rem !important; }
...
st.markdown("<p style='font-size: 14px; color: gray;'>version 116.3 (UI Restored) พัฒนาโดย Niratsai Sukprasert และ Gemini</p>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with st.expander("คลิกเพื่อระบุผู้ลางาน (สูงสุด 5 คน)", expanded=False):
        for i in range(5):
            ...
            with c1: p_leave = st.selectbox(f"ชื่อ", dropdown_names, key=f"l_name_{i}", label_visibility="collapsed")
    ...
    with st.expander("คลิกเพื่อล็อกภาระงานหลัก (สูงสุด 20 รายการ)", expanded=False):
        main_task_display_options = [
            'จ่าย 4', 'จ่าย 5', 'จ่าย 6', 'จ่าย 7', 'จ่าย 8', 'จ่าย 9', 'จ่าย 10', 'จ่าย 11',
            'Ver 1 INC', 'Ver 2/ปณ.', 'Ver 3/A', 'Ver 4', 'Ver 5', 'Ver 6', 'Ver 7', 'Ver 8', 'Ver 9', 'Ver 10',
            'Ver PS1', 'Ver PS2', 'Ver PS3', 'Ver PS4', 'Ver PS5', 'Ver PS6', 'Ver PS7', 'Ver PS8', 'Ver PS9', 'Ver PS10',
            'Match + C', 'Match + C2', 'Matching'
        ]
        task_name_mapping = {
        ...
        for i in range(20):
            ...
            n_m_task = st.selectbox(f"ภาระงาน", ["เลือกภาระงาน"] + main_task_display_options, key=f"m_task_{i}", label_visibility="collapsed")
            c1, c2 = st.columns(2)
            ...
            if p_m_task != "ไม่มี" and n_m_task != "เลือกภาระงาน":
                if VALID_TIMES.index(s_m_task) < VALID_TIMES.index(e_m_task):
                    internal_task_name = task_name_mapping[n_m_task]
                    fixed_main_tasks_input[(p_m_task, s_m_task, e_m_task)] = internal_task_name
                else:
    ...
    with st.expander("คลิกเพื่อล็อกเวลาพัก (สูงสุด 5 คน)", expanded=False):
        ...
        for i in range(5):
            ...
            with c1: p_b = st.selectbox(f"ชื่อ", dropdown_names, key=f"b_name_{i}", label_visibility="collapsed")
...
if st.session_state.schedule_df is not None and st.session_state.run_status == "Success":
    ...
    try:
        styled_df = df_to_show.style.map(get_color_style, subset=df_to_show.columns[1:])
    except AttributeError:
        styled_df = df_to_show.style.applymap(get_color_style, subset=df_to_show.columns[1:])
